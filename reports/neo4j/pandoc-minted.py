...
def unpack_code(value, language):
    ...
    attributes.append(["framesep", "2mm"])
    attributes.append(["numbersep", "5pt"])
    attributes.append(["fontsize", "\\footnotesize"])
    # attributes.append(["linenos"])
    attributes.append(["mathescape"])
    attributes.append(["bgcolor", "codebg"])

    attributes = ', '.join(parse_attrib(x) for x in attributes)
